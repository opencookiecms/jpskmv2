...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "conformupdate")) {
   ...
   copy($src,$gambar);
  $updateSQL = sprintf("UPDATE kontraktor SET konImage='$gambar' WHERE kontraktorId=%s",
...
$colname_updategameber = "-1";
if (isset($_GET['kontraktorId'])) {
  $colname_updategameber = $_GET['kontraktorId'];
}
mysql_select_db($database_dbconn, $dbconn);
$query_updategameber = sprintf("SELECT * FROM kontraktor WHERE kontraktorId = %s", GetSQLValueString($colname_updategameber, "int"));
$updategameber = mysql_query($query_updategameber, $dbconn) or die(mysql_error());
$row_updategameber = mysql_fetch_assoc($updategameber);
$totalRows_updategameber = mysql_num_rows($updategameber);
?>
<!DOCTYPE html>
<html>
<head>
...
<?php include '../include/headers.php';?>
  ...
  <div class="col-md-5">
                                              <label>Kontraktor ID</label>
                                             	<a class="showid">:<?php echo $row_updategameber['kontraktorId']; ?></a>
                                              <input name="idKontraktor" class="form-control hidden" type="text"value="<?php echo $row_updategameber['kontraktorId']; ?>">
                                              </div>
    ...
    <div class="col-lg-3">
      ...
      <label><?php echo $row_updategameber['konName']; ?></label>
    </div>
  </div>
  <div class="form-group">
    <div class="col-lg-3">
      ...
      <label>Gambar</label>
      <input name="file" type="file" class="form-control" accept="image/*" value="">
    </div>
  </div>
